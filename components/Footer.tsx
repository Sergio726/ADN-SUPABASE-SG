export default function Footer() {
  return (
    <footer className="bg-brand-black text-white py-12 mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="md:col-span-2">
              <img 
                src="/logos/logo-grey.png" 
                alt="Alambres del Norte" 
                className="h-60 w-auto mb-4"
              />
            <p className="text-gray-400 max-w-md">
              Fabricación, instalación y suministro de cercos de calidad. 
              Más de 6 años de experiencia en el mercado.
            </p>
            <div className="flex space-x-4 mt-4">
              <a 
                href="https://www.instagram.com/alambresdelnorte/" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-brand-red transition-colors"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073z"/><path d="M12 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
              </a>
              <a 
                href="https://www.facebook.com/alambresdelnorte" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-brand-red transition-colors"
              >
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>
          
          <div>
            <h4 className="text-lg font-semibold mb-4 text-brand-red">Contacto</h4>
            <ul className="space-y-3 text-gray-400">
              <li className="flex items-start">
                <span className="mr-2">📞</span>
                <div>
                  <a href="tel:+5493874773393" className="hover:text-white font-semibold">+54 9 3874 77-3393</a><br/>
                  
                </div>
              </li>
              <li className="flex items-start">
                <span className="mr-2">📧</span>
                <a href="mailto:amiranda@alambresdelnortesrl.com.ar" className="hover:text-white break-all">
                  amiranda@alambresdelnortesrl.com.ar
                </a>
              </li>
              <li className="flex items-start">
                <span className="mr-2">📍</span>
                <span>Gral. Arias Rangel 320<br/>(Alt. Zuvirí al 2000)<br/>Salta, Argentina</span>
              </li>
            </ul>
          </div>
          
          <div>
            <h4 className="text-lg font-semibold mb-4 text-brand-red">Horarios</h4>
            <ul className="space-y-2 text-gray-400">
              <li>Lunes a Viernes<br/><span className="text-white">8:00 - 18:00</span></li>
              <li>Sábados<br/><span className="text-white">8:00 - 13:00</span></li>
              <li>Domingos<br/><span className="text-white">Cerrado</span></li>
            </ul>
          </div>
        </div>
        
        <div className="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400">
          <p>&copy; {new Date().getFullYear()} Alambres del Norte SRL. Todos los derechos reservados.</p>
          <p className="mt-2 text-sm">
            <a href="https://www.alambresdelnortesrl.com.ar" className="hover:text-brand-red">
              www.alambresdelnortesrl.com.ar
            </a>
          </p>
        </div>
      </div>
    </footer>
  )
}

